...
ILR.fit(pd.DataFrame({1:int_x},dtype='O'),y,sample_weight=w,fast=True,n_p_vals=10,tqdm_disable=True)
...
tikzplotlib.save('figs/apx_log4-lines_all.tikz',figure = fig1,externalize_tables = True, override_externals = True,tex_relative_path_to_data = 'dat/apx/')
# %%
def func(value):
    ...
    y = rng.choice(y_vals)
    # rng.shuffle(y)
    x = np.sort(rng.uniform(0,4,few))[::rng.choice((-1,1))]
    int_x = [pba.I(i,i+1) for i in x]
    
    ILR = ImpLogReg(uncertain_data=True)
    ILR.fit(pd.DataFrame({1:int_x},dtype='O'),pd.Series(y),sample_weight=w,fast=True,n_p_vals=100,tqdm_disable = True)
    yl = [i.left for i in ILR.predict_proba(xl.reshape(-1, 1))[:,1]]
    yr = [i.right for i in ILR.predict_proba(xl.reshape(-1, 1))[:,1]]
...
if __name__ == '__main__':
    ...
    fig_hist.savefig('hist1.png')
    tikzplotlib.save("figs/apx_log4-lines_hist.tikz",figure=fig_hist)
# %%
