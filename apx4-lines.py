...
y = pd.Series([0,0,1,1])
int_x = [pba.I(1.5,2.5),pba.I(1.9,2.9),pba.I(3,4),pba.I(3.5,4.5)]
...
ILR.fit(pd.DataFrame({1:int_x},dtype='O'),y,sample_weight=w,fast=True,n_p_vals=10)
...
def func(value):
    ...
    ILR.fit(pd.DataFrame({1:int_x},dtype='O'),pd.Series(y),sample_weight=w,fast=True,n_p_vals=10)
    yl = [i.left for i in ILR.predict_proba(xl.reshape(-1, 1))[:,1]]
    yr = [i.right for i in ILR.predict_proba(xl.reshape(-1, 1))[:,1]]
