...
def generate_results(data):
    ...
    for row in data.index:
        ...
        results[row] = sum(data.loc[row]) >= len(data.columns)*(1.5+np.random.rand()/3)
    
    return results

def int_logistic_regression(data,result,uncertain):
...
# Generate data
data = pd.DataFrame(3*np.random.rand(many,dim))
results = generate_results(data)

# Generate uncertain points
uncertain = 0.5+pd.DataFrame(2*np.random.rand(few,dim))
...
# Fit model
models = int_logistic_regression(data,results,uncertain)
