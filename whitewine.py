...
#%%
### Import the data
whitewine = pd.read_csv('redwine.csv',index_col = None)
X = whitewine[[c for c in whitewine.columns if c not in ['quality']]]
Y = whitewine['quality'] >= 7

#%%
### Split into test and train samples
train_data, test_data, train_results, test_results = train_test_split(X[["volatile acidity","residual sugar","chlorides","density","pH","alcohol"]], Y, test_size=0.9,random_state = 0,stratify=Y)
...
#%%
### Fit UQ models
ilr = ImpLogReg(uncertain_data=True, max_iter = 10000)
